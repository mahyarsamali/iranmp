...
public:
    CSourceLine(const SString& strText) { SetFromString(strText); }
    void SetFromString(const SString& strText) { strText.Replace("\t", " ").Replace("  ", " ", true).TrimEnd(" ").TrimStart(" ").Split(" ", m_TokenList); }
...
public:
    ...
    int GetVariableInt(const SString& strName) { return atoi(GetVariable(strName)); }
    ...
    // Status
    bool IsLineValid(const SString& strLine) { return !CSourceLine(strLine).GetToken(0).empty(); }
